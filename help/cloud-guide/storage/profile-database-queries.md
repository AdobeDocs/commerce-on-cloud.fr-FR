---
title: Requêtes de base de données de profils
description: Découvrez comment activer le profilage pour comprendre l’impact des modifications apportées à la base de données.
...
# Requêtes de base de données de profils

Lorsque vous écrivez dans une base de données, il peut être utile d’exécuter le profilage pour identifier l’impact de ces modifications. Le profilage conserve un journal des requêtes de base de données et augmente la précision d’exécution.
...
1. Afficher le journal des requêtes.

   ```sql
   SHOW profiles
   ```

1. Effacez la file d’attente des profils.
