...
- **Terminer les tâches après la mise à niveau**-
  ...
  - Utilisez SSH pour vous connecter au serveur distant et vérifier les éléments suivants :
    ...
    - Vérifiez le statut de l’indexeur et réindexez-le si nécessaire. Voir [Gestion des indexeurs](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/manage-indexers) dans le _Guide de configuration_.
    ...
    - Vérifiez les journaux `cron` et la table `cron_schedule` dans la base de données Adobe Commerce pour connaître l’état cron, puis réexécutez les tâches cron si nécessaire.
      Voir [Journalisation](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/configure-cron-jobs#logging) dans le _Guide de configuration_.
