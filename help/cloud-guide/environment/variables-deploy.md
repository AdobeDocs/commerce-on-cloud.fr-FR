...
- `false` : `Consumers` traiter les messages disponibles dans la file d&#39;attente, fermer la connexion TCP et terminer. `Consumers` n’attendez pas que d’autres messages rejoignent la file d’attente, même si le nombre de messages traités est inférieur à la valeur de `max_messages` spécifiée dans la variable de déploiement `CRON_CONSUMERS_RUNNER`.
